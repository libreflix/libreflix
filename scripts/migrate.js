...
function runMigrations() {
    ...
    if (command === 'up') {
        ...
        return getAppliedMigrations()
            .then(function (docs) {
                applied = docs.map(function (doc) { return doc.name; });
                ...
                if (pending.length === 0) {
                    console.log(targetId ? 'Migration ' + targetId + ' already applied or not found.' : 'No pending migrations.');
                    return;
                }
        ...
        return Migration.findOne(query).sort({ appliedAt: -1 }).exec()
            .then(function (migrationDoc) {
                if (!migrationDoc) {
                    console.log(targetId ? 'Migration ' + targetId + ' not found or not applied.' : 'No migrations to rollback.');
                    return;
                }

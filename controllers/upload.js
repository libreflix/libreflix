...
const { buildUploadedFileName } = require('../utils/form-util');
...
exports.uploadImage = function (req, res) {
    ...
    form.on('file', function (field, file) {
        fileEndName = uuidV1() + "-libreflix" + path.extname(file.name);

        //--------------------------------------------------------------------------
        // COWNOTE(n2omatt): fs.rename fails on cross device files.
        //   So if the tmpfs is mounted on /tmp (which is to be the default)

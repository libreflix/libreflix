    ...
    if (film) {
      Category.find({}, function (err, categories) {
        Comment.find({ attachedToWatch: film.id }, function (err, comments) {
          Interaction.find(
            { attachedToWatch: film.id },
            function (err, all_rating) {
              Interaction.findOne(
                { proofhash: phash },
      ...
      if (req.body.comment_body) {
        ...
        if (req.body.comment_body.trim() !== "" || req.body.stars > 0) {
          comment.save(function (err) {
            ...
            req.flash("success", {
              msg: "Seu comentário e/ou avaliação foram postados com sucesso.",
            });
          ...
          req.flash("error", {
            msg: "Seu comentário e/ou avaliação estão vazios.",
          });
          res.redirect(req.get("referer"));
        }
      }
      // if is a star rating
        ...
        if (interaction) {
          ...
          interaction.save(function (err) {
            //req.logIn(campanha, function(err) {
            req.flash("success", {
              msg: "Sua avaliação foi atualizada com sucesso.",
            });
            res.redirect(req.get("referer"));
            //});
          });
          ...
          interaction.save(function (err) {
            //req.logIn(campanha, function(err) {
            req.flash("success", {
              msg: "Sua avaliação foi postada com sucesso.",
            });
            res.redirect(req.get("referer"));
            //});
          });
  ...
  Comment.findOne({ _id: req.body.del }, function (err, comment) {
    if (comment.creator == req.user.id || req.user.adm) {
      Comment.remove({ _id: req.body.del }, function (err) {
        ...
        res.redirect(
          req.originalUrl.replace("?_method=DELETE", "") + "#comments",
        );
      });
    }
  });
